...
PurePursuit::PurePursuit(ros::NodeHandle& nh) : nh_(nh)
{
  ...
  if (!nh.param<int>("controller/steering/look_ahead_distance", look_ahead_distance, 10))
  {
    ROS_WARN_STREAM("Did not load controller/steering/look_ahead_distance. Standard value is: " << 10);
  }
};

// Getters
fsd_common_msgs::ControlCommand PurePursuit::getControlCommand() const
{
...
void PurePursuit::createControlCommand()
{
  ...
  // point on center line next to car
  const auto it_center_line =
      std::min_element(center_line_.points.begin(), center_line_.points.end(),
  ...
  const auto i_next = (i_center_line + look_ahead_distance) % size;
  ...
  {  // Steering Control
    ...
    try
    {
      geometry_msgs::PointStamped next_point_odom, next_point_fscar;
      next_point_odom.point.x = next_point.x;
      next_point_odom.point.y = next_point.y;
      next_point_odom.point.z = next_point.z;
