...
// Constructor
PurePursuitHandle::PurePursuitHandle(ros::NodeHandle& nodeHandle) : nodeHandle_(nodeHandle), pure_pursuit_(nodeHandle)
{
...
// Getters
int PurePursuitHandle::getNodeRate() const
{
  return node_rate_;
}
...
void PurePursuitHandle::loadParameters()
{
  ...
  if (!nodeHandle_.param<std::string>("velocity_estimate_topic_name", velocity_estimate_topic_name_,
  ...
  if (!nodeHandle_.param<std::string>("control_command_topic_name", control_command_topic_name_,
  ...
  if (!nodeHandle_.param<std::string>("center_line_topic_name", center_line_topic_name_,
...
void PurePursuitHandle::slamMapCallback(const fsd_common_msgs::Map& map)
{
  ...
  {  // Find Center Line
    ...
    for (const auto& yellow : map.cone_yellow)
    {
      const auto it_blue = std::min_element(
          map.cone_blue.begin(), map.cone_blue.end(),
  ...
  center_line.header.frame_id = "odom";
